...
<html>
<head>
    <title>校园二手书交易平台后台管理系统——用户管理</title>
    ...
    <nav class="navbar">
        ...
        <form action="searchBook.do" method="post">
            <div class="nav-search">
                <a href="home.do"><img class="logo" src="<%=request.getContextPath()%>/img/adminlogo.png"></a>
        ...
        <ul class="menu">
            ...
            <li><a class="active" href="/admin/goadminuser">用户管理</a></li>
...
<div id="container">
    ...
    <div id="book-container">
        <h3 class="book-title"><a href="#">| 管理所有用户 </a></h3>
        <ul class="book-lists">
            <c:forEach items="${users}" var="user" varStatus="userStatus">
                <li class="book-list">
                        ...
                        <%--                    </span>--%>
                        <%--                    <h5 class="book-name">${user.Todetail()}</h5>--%>
                        <%--                    <li>--%>
                    <h5 class="book-name">用户ID：${user.id}</h5>
                    <h5 class="book-name">用户名：${user.studentid}</h5>
                    <h5 class="book-name">姓名：${user.name}</h5>
                    <h5 class="book-name">登录口令：${user.password}</h5>
                    <h5 class="book-name">性别：${user.sex}</h5>
                    <h5 class="book-name">联系方式：${user.tel}</h5>
                    <h5 class="book-name">用户住址：${user.address}</h5>
                    <h5 class="book-name">用户专业：${user.major}</h5>
                        <%--                        <h5 class="book-name">${user}</h5>--%>
                    <span class="delete-user" user-id = ${user.id}>
						    <a href="#" class="book-buy">删除用户</a>
                        </span>
                        <%--                    </li>--%>
...
<script type="text/javascript">
    ...
    $("span.delete-user").on("click",function(){
        var $this = $(this);
        var Mydata="id="+$this.attr("user-id");
        $.ajax({
            type:"POST",
            url:"/admin/goadminuser",
            data:Mydata,
            success:function(result){
                alert("删除成功");
                window.location.href="/admin/goadminuser";
            },
