...
<div class="card-btn">
    ...
    <span title="publish-resource" id="resource-btn">我的资源</span>
    <p class="upload-icon"></p>
    <a id="resource-upload" href="/resource/upload">上传资源</a>
        ...
        <ul class="book-ul">
            <c:forEach items="${resourceList}" var="resource" varStatus="rscStatus">
                <li class="book-list">
                    ...
                    <span class="rsc-delete-btn" data-id="${resource.getId()}"></span>
                    <div class="book-detail">
                        <p>描&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;述：<span class="book-author">${resource.getDescription()}</span></p>
                    </div>
                </li>
            </c:forEach>
        </ul>
    </div><!-- resource-part -end-->
</div> <!-- container-end -->
...
<script>
    var resourceId;
    $("span.rsc-delete-btn").on("click",function(){
        var $this = $(this);
        var Mydata="resourceId="+$this.attr("data-id");
        $.ajax({
            type:"POST",
            url:"/resource",
            data:Mydata,
